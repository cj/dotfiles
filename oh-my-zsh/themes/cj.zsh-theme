...
ZSH_THEME_GIT_PROMPT_AFTER="%{$fg[cyan]%} ⓣ"

# Format for git_prompt_long_sha() and git_prompt_short_sha()
ZSH_THEME_GIT_PROMPT_SHA_BEFORE="%{$fg[white]%}::%{$reset_color%} %{$fg_bold[yellow]%}"
...
PROMPT='
%{$fg[cyan]%}%~ %{$reset_color%}$(git_prompt_long_sha)$(git_prompt_info)
%{$reset_color%} $(prompt_char) $(git_time_since_commit)$(git_prompt_status)%{$reset_color%} : '
